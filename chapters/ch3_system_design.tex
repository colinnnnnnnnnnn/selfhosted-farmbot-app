...
\subsection{Extended Platform Services}

To enhance system reliability, security, and user engagement, several platform-level services have been integrated into the FarmBot ecosystem. These additions strengthen the operational architecture and support modern software engineering standards.

\paragraph{OAuth2 Authentication.}
A secure OAuth2-based authentication mechanism has been incorporated to ensure robust access management while allowing seamless integration with third-party identity providers. This enhancement improves user security and enables scalable multi-user access.

\paragraph{Audit Logging.}
A centralized audit logging subsystem records all critical system actions, including user commands, configuration changes, and automated device events. This provides complete traceability, supports debugging, and strengthens compliance with best practices for safety-critical automation systems.

\paragraph{Email Notification Service.}
An automated email notification module has been introduced, enabling the system to alert users about key events such as task completion, sensor anomalies, low moisture levels, or critical failures. This improves real-time awareness and reduces dependency on manual monitoring.
...
\subsection{Sequence Editor Integration}

A sequence editor has been integrated into the FarmBot control interface, enabling users to design, customize, and automate task sequences without manual coding. This tool allows the creation of multi-step procedures such as planting cycles, irrigation routines, and maintenance operations.

The editor supports drag-and-drop actions, parameterized steps, conditional execution, and the scheduling of sequences. Users can define variables, set step-specific parameters (e.g., duration, target coordinates, tool selection), and include conditional branches based on sensor readings or system state. Scheduling capabilities permit recurring or timed execution, and a preview/simulation mode lets users validate sequences before deployment.

This integration improves usability and empowers non-technical users to automate complex workflows with precision and consistency, while maintaining traceability and safety through built-in validation and logging.
...
\subsection{Console Output Stream}

To support real-time diagnostics and improve developer visibility, a dedicated console output stream has been developed. This stream captures device logs, sensor readings, movement reports, and error messages directly from the hardware controller.

The console is integrated with the backend through a WebSocket channel, enabling live monitoring via the web interface. This feature significantly enhances debugging efficiency and facilitates rapid detection of hardware or communication issues.
...
\section{High-Level API and Hardware Control Layer}

A robust Python-based control layer has been developed to simplify interactions with the FarmBot hardware. This API wrapper consolidates low-level device operations into an accessible library and exposes high-level routines for common tasks.

Key features include:
\begin{itemize}
  \item Soil moisture sensing using analog pin 59.
  \item Seed injector control via digital pin 10.
  \item Rotary tool management through analog pin 11.
  \item High-level functions for watering routines, weeding sequences, and photo capture.
  \item Device state queries, movement commands, homing procedures, and emergency handling.
  \item Automatic retrieval of photo URLs using MQTT event logs with a fallback to API queries.
\end{itemize}

The hardware control layer integrates cleanly with the Django backend, enabling remote control, automation workflows, and scheduled actions. This abstraction simplifies system development while enforcing safety checks and consistent operation across all hardware modules. Comprehensive unit tests and simulation hooks are provided to validate routines before deployment to physical devices.
